...
들여쓰기는 반드시 공백이 아닌 탭('\t')으로 사용한다.  

특별한 경우가 아니면 "줄의 시작" 지점은 들여쓰기가 시작되는 부분을 의미한다. 예를 들어 들임 2단계가 적용된 줄 "\\t\\tAbc..." 에서 그 줄의 시작 문자는 "\\t"가 아닌 "A"이다.  

## 제외 대상

다음의 경우는 위에서 언급한 두 규칙(줄길이제한, 탭으로 들여쓰기)만 적용하며 이 후 언급하는 모든 규칙은 적용하지 않는다.
- 주석
- 문자열 리터럴
...
- 위에서 언급한 제외 대상이 아니라면 들여쓰기 이외의 용도로 탭('\t')을 쓰지 않는다. 
- 공백(' ')은 연속으로 4번 이상 붙여 쓰지 않는다. 

## 괄호 규약

### 제외 대상
다음 중 하나라도 해당한다면 `(`, `)`, `{`, `}`, `[`, `]`, `<`, `>` 등의 문자가 있다 하더라도 괄호 표현으로 간주하지 않으며 후술할 괄호 표현과 관련된 어떠한 규칙에도 적용되지 않는다.
...
```
여는 괄호 바로 다음에 개행이 온다면 이후의 다른 줄 어딘가에 존재하는 닫는 괄호 문자 까지 범위에 대해 다중행 규칙을 적용한다.
...
- 단, n > 3 인 경우는 공백 제한을 위반하므로 여닫는 괄호를 다른 줄에 배치 후 다중행 규칙을 적용한다.

### 다중행
열고 닫는 괄호가 서로 다른 줄에 있는 괄호 표현식에 대해 다음 규칙을 적용한다.
- 열고 닫는 괄호가 있는 두 줄의 들임 깊이는 같아야 하고, 그 사이에 있는 줄들의 들임 깊이는 여닫는 괄호가 있는 줄의 그것보다 한 단계 깊게 시작한다.
...
가상 괄호는 실제 코드에는 보이지 않고 자리도 차지하지 않지만, 위에서 언급한 괄호 규칙이 동일하게 적용되는 괄호다(단, 일반 괄호들과 달리 글자 수에 포함하지 않는다). 가상 괄호는 특정 구문에서 특정 키워드 다음에만 발생할 수 있고 그 키워드들을 가상 괄호 키워드라고 한다. 가상 괄호가 발생할 수 있는 구문과 그 키워드는 다음과 같다.(구문 / "키워드")
...
### 선언문
- 선언문 전체(타입부터 종료 세미콜론까지)가 두 줄 이상을 차지한다면 위에서 언급한 가상 괄호 규칙을 먼저 적용한다. 그리고 그 가상 괄호 내 스코프 안에서는 기본적으로 앞서 언급한 표현식 규칙을 적용한다.   
